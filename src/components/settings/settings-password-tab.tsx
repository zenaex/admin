"use client";

import { useEffect, useState } from "react";
import { SettingsPasswordReset } from "@/components/settings/settings-password-reset";
import { ResetRequestsTable } from "@/components/settings/settings-reset-requests";

type PasswordSubTab = "reset" | "requests";

type SettingsPasswordTabProps = {
  /** Super admin only: list and moderate password reset requests. */
  showResetRequests: boolean;
};

export function SettingsPasswordTab({ showResetRequests }: SettingsPasswordTabProps) {
  const [subTab, setSubTab] = useState<PasswordSubTab>("reset");

  useEffect(() => {
    if (!showResetRequests && subTab === "requests") setSubTab("reset");
  }, [showResetRequests, subTab]);

  return (
    <div>
      {showResetRequests ? (
        <div className="mb-6 flex items-center gap-6 rounded-full border border-outline bg-white px-5 py-3.5">
          {(["reset", "requests"] as PasswordSubTab[]).map((id) => {
            const label = id === "reset" ? "Password Reset" : "Reset Requests";
            const active = subTab === id;
            return (
              <button
                key={id}
                type="button"
                onClick={() => setSubTab(id)}
                className={`text-sm transition-colors ${
                  active
                    ? "rounded-full bg-zinc-200 px-4.5 py-1.5 font-semibold text-primary-text"
                    : "font-medium text-zinc-400 hover:text-zinc-600"
                }`}
              >
                {label}
              </button>
            );
          })}
        </div>
      ) : null}

      {subTab === "reset" && <SettingsPasswordReset />}
      {showResetRequests && subTab === "requests" && <ResetRequestsTable />}
    </div>
  );
}
